...
const House = ({ house, state, updateState, getList }) => {
    ...
    const handleAddResident = async (houseAddres) => {

        const { value: formValues } = await Swal.fire({
            title: "Asignar  nuevo residente",
            html: `
                <div class="flex flex-col">
                    <label for="swal-input2">Nuevo residente:</label>
                    <input id="swal-input2" type="email" class="swal2-input peer disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
                        invalid:border-red-500 invalid:text-red-600
                        focus:invalid:border-red-500 focus:invalid:ring-red-500 rounded-lg">
                        <p class="invisible peer-invalid:visible text-red-500 text-sm">Ingrese un email valido.</p>
        ...
        if (formValues) {
            handleConnectResident(houseAddres, formValues.email1);
        }
    }

    const handleConnectResident = async (houseAddres, email) => {
        ...
        <div>
            <div className='flex-col pb-1' >
                <div className='flex justify-between pt-5'>
                    ...
                    <div onClick={() => handleAddResident(house.address)} className={` flex items-center justify-end pr-2 group cursor-pointer transition ease-in-out hover:-translate-y-1 hover:scale-100  ${state ? '' : 'hidden'}`}>
